...
/*! \def PCD_PRINTF_INFO_LOGFILE
 *  \brief Print a message INFO level to log file
 */
#define PCD_PRINTF_INFO_LOGFILE( _format, _args... )        \
do { char tmpLogBuffer[ CONFIG_PCD_MAX_LOG_SIZE ]; \
    snprintf( tmpLogBuffer, CONFIG_PCD_MAX_LOG_SIZE - 1, "%s%s"_format "%s", PCD_PRINT_PREFIX, "Info: ", ##_args, ".\n" ); \
	if( verboseOutput ) fprintf( stdout, "%s", tmpLogBuffer ); \
    PCD_errlog_log( tmpLogBuffer, True ); \
} while( 0 )

/*! \def PCD_PRINTF_WARNING_LOGFILE
 *  \brief Print a message WARNING level to log file
 */
#define PCD_PRINTF_WARNING_LOGFILE( _format, _args... )        \
do { char tmpLogBuffer[ CONFIG_PCD_MAX_LOG_SIZE ]; \
    snprintf( tmpLogBuffer, CONFIG_PCD_MAX_LOG_SIZE - 1, "%s%s"_format "%s", PCD_PRINT_PREFIX, "Warning: ", ##_args, ".\n" ); \
	if( verboseOutput ) fprintf( stderr, "%s", tmpLogBuffer ); \
    PCD_errlog_log( tmpLogBuffer, True ); \
} while( 0 )

/*! \def PCD_PRINTF_FATAL_LOGFILE
 *  \brief Print a message FATAL level to log file
 */
#define PCD_PRINTF_FATAL_LOGFILE( _format, _args... )        \
do { char tmpLogBuffer[ CONFIG_PCD_MAX_LOG_SIZE ]; \
    snprintf( tmpLogBuffer, CONFIG_PCD_MAX_LOG_SIZE - 1, "%s%s"_format "%s", PCD_PRINT_PREFIX, "Fatal: ", ##_args, ".\n" ); \
	if( verboseOutput ) fprintf( stderr, "%s", tmpLogBuffer ); \
    PCD_errlog_log( tmpLogBuffer, True ); \
} while( 0 )
