...
/**************************************************************************/
/*! \fn PCD_api_send_process_ping()								*/
/**************************************************************************/
/*  \brief 		Send PROCESS_PING event to PCD                        *
 *  \param[in] 		None
 *  \param[in,out] 	None										*
 *  \return			PCD_STATUS_OK - Success, <0 - Error	               *
 **************************************************************************/
PCD_status_e PCD_api_send_process_ping( void )
{
    return PCD_api_malloc_and_send( NULL, PCD_API_PROCESS_PING, NULL, getpid() );
}
...
PCD_status_e PCD_api_register_exception_handlers( char *name, Cleanup_func cleanup )
{
    ...
    sprintf( btTmpFile, "%s/%d.bt", CONFIG_PCD_TEMP_PATH, pid );
...
/**************************************************************************/
/*  \brief Find process ID, detects if another instance already running.   *
 ...
 *  \param[in] 		Reboot reason (optional)       						  *
 ...
 *  \brief Reduce net-rx task priority to a given priority value (non preemptive mode)
