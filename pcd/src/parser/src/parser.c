...
static int32_t PCD_parser_generate_config_in_dir( const char *service_name )
{
	char *filename;
	DIR *dirfp;
	...
	if (! filename) {
      PCD_PRINTF_STDERR( "No memory!" );
	  return -1;
	}
	errno = 0;
	dirfp = opendir(service_name);
	if (! dirfp) {
      PCD_PRINTF_STDERR( "Unable to read included directory: %s", service_name);
	  free(filename);
	  return -1;
	}
	/* Get the list of files in the directory */
	while ((direntry = readdir(dirfp)) != 0) {
	  storename = direntry->d_name;

	  /* Don't try to parse any dot file ('.' and '..')
	  * without the '.pcd' suffix or ending with a tilde ('~').
	  * This prevents the parsing of
	  * many editor files, temporary files and those saved by RPM
	  * package upgrades.
	  */
	  if ( !storename[0] /* Shouldn't happen */ ||
			  storename[0] == '.' ||
			  strstr(storename, ".pcd") == NULL ||
			  storename[strlen(storename)-1] == '~') {
		 continue;
	  }

	  snprintf(filename, len_sn+NAME_MAX+1, "%s/%s",
		 service_name, storename);

	  if( stat(filename, &sb) < 0 ) {
	     PCD_PRINTF_STDERR( "Unable to stat includedir file %s. Skipped.", filename);
		 continue;
	  }

	  /* Only open it if it's a regular file. */
	  if( !S_ISREG(sb.st_mode) ) {
		  PCD_PRINTF_STDERR( "%s is not a regular file. It is being skipped.", filename );
		  continue;
	  }

	  PCD_DEBUG_PRINTF( "Reading included configuration file: %s",filename );
      if ( PCD_parser_read_config( filename, False ) )
      {
	...
	closedir(dirfp);
	if ( errno != 0) {
		PCD_PRINTF_STDERR( "Error reading included directory: %s", service_name);
	}
...
static int32_t PCD_parser_generate_config( const char *filename )
{
    ...
    if( stat(filename,&s) == 0 )
    {
    	if ( S_ISLNK(s.st_mode) )
		{
			...
			{
		        //error
		        PCD_PRINTF_STDERR( "Can't stat the file/directory behind the link %s", filename );
        ...
        {
            //it's a directory
            if ( PCD_parser_generate_config_in_dir( filename ) )
            {
                PCD_PRINTF_STDERR( "Reading the input configuration" );
                goto generate_config_exit;
            }
        ...
        {
            //something else
            PCD_PRINTF_STDERR( "Stat file/directory %s as other", filename );
            goto generate_config_exit;
        }
    }
    else
    {
        //error
        PCD_PRINTF_STDERR( "Can't stat the file/directory %s", filename );
        goto generate_config_exit;
    }
...
static int32_t PCD_parser_handle_FAILURE_INTERVAL( char *line )
{
    PCD_FUNC_ENTER_PRINT
    ...
    {
        errno = 0;
        rule.fail_interval = strtoul( line, (char**)NULL, 0 );
        if ( errno != 0 )
        {
             PCD_PRINTF_STDERR( "FAILURE_INTERVAL numeric value is invalid for rule = %s",
                               rule.ruleId.ruleName );
        }
    }
    else
    {
        PCD_PRINTF_STDERR( "FAILURE_INTERVAL is not a numeric entry for rule = %s",
                           rule.ruleId.ruleName );
        return -1;
    }
    return 0;
...
static int32_t PCD_parser_handle_FAILURE_RESET_COUNT( char *line )
{
    ...
    {
        errno = 0;
        rule.fail_reset_count = strtoul( line, (char**)NULL, 0 );
        if ( errno != 0 )
        {
             PCD_PRINTF_STDERR( "FAILURE_RESET_COUNT numeric value is invalid for rule = %s",
                               rule.ruleId.ruleName );
        }
    }
    else
    {
        PCD_PRINTF_STDERR( "FAILURE_RESET_COUNT is not a numeric entry for rule = %s",
                           rule.ruleId.ruleName );
        return -1;
    }
    return 0;
...
static int32_t PCD_parser_handle_ALIVE_TIMEOUT( char *line )
{
    int32_t i = atoi( line );
...
static int32_t PCD_parser_handle_FAILURE_ACTION( char *line )
{
    ...
    if ( i == PCD_FAILURE_ACTION_KEYWORD_RESTART )
    {
    	if ( token2 )
    	{
            errno = 0;
            rule.faulty = strtoul( token2, (char**)NULL, 0 );
            if ( errno != 0 )
            {
                 PCD_PRINTF_STDERR( "RESTART numeric value is invalid for rule = %s",
                                   rule.ruleId.ruleName );
                 rule.faulty = -1;
            }
    	} else {
    ...
    if ( i == PCD_FAILURE_ACTION_KEYWORD_EXEC_RULE )
    {
        ...
        return PCD_parser_parse_rule_id( &rule.failureAction[0].ruleId, token2 );
    }

    return 0;
}

static int32_t PCD_parser_handle_FAILURE_ACTION2( char *line )
{
    char *token1, *token2;
    u_int32_t i = 0;
    ...
    if ( i == PCD_FAILURE_ACTION_KEYWORD_EXEC_RULE )
    {
        ...
        return PCD_parser_parse_rule_id( &rule.failureAction[1].ruleId, token2 );
    }

    return 0;
}

static int32_t PCD_parser_handle_FAILURE_ACTION_NEXT( char *line )
{
    char *token1, *token2;
    u_int32_t i = 0;
    ...
    if ( i == PCD_FAILURE_ACTION_KEYWORD_EXEC_RULE )
    {
        ...
        return PCD_parser_parse_rule_id( &rule.failureAction[2].ruleId, token2 );
    }
