...
bool_t PCD_timer_iterate( void )
{
    ...
    while ( searchList )
    {
        ...
        //count the time before reset fail count
        if ( rule->fail_reset == 0 )
        {
        	rule->fail_count = 0;
        	rule->fail_reset = rule->fail_reset_count * 3600000;
        } else {
            if ( rule->fail_reset <= PCD_TIMER_TICK )
            {
            ...
            {
                /* Reduce a tick */
            	rule->fail_reset -= PCD_TIMER_TICK;
            }
        }
...
static timerObj_t *PCD_timer_new( rule_t *rule )
{
    ...
    newObj->failureActionFunc = PCD_failure_action_get_function( rule->failureAction[(rule->fail_count>2?2:rule->fail_count)].action );
    newObj->rule = rule;
    newObj->timeout = rule->timeout;
    newObj->alive_timeout = rule->alive_period;
    newObj->starttime = rule->fail_interval * 1000;
    newObj->prev = NULL;
    newObj->next = NULL;
...
static bool_t PCD_timer_handle_start_condition( timerObj_t *timerObj )
{
    ...
    if ( timerObj->starttime == 0 )
    {
        ...
        if ( timerObj->starttime <= PCD_TIMER_TICK )
        {
            /* Round timeout for the next iteration */
        ...
        {
            /* Reduce a tick */
            timerObj->starttime -= PCD_TIMER_TICK;
        }
...
static bool_t PCD_timer_handle_end_condition( timerObj_t *timerObj )
{
    ...
    if ( rule->alive_period != 0 )
    {
    	if ( timerObj->alive_timeout == 0 )
    	{
    		if ( rule->ping != rule->ping_mem )
    		{
    			rule->ping_mem = rule->ping;
    			timerObj->alive_timeout = rule->alive_period;
    		}
    		else
    		{
    			//must stop because hanged
    	        PCD_PRINTF_STDERR( "Rule %s_%s: Hung", rule->ruleId.groupName, rule->ruleId.ruleName );

    	        /* If a process is running, kill it */
    	        if ( rule->proc )
    	            PCD_process_stop( rule, True, NULL );

    	        rule->ruleState = PCD_RULE_HUNG;
			...
			if ( timerObj->alive_timeout <= PCD_TIMER_TICK )
			{
				/* Round timeout for the next iteration */
				timerObj->alive_timeout = 0;
			}
			else
			{
				/* Reduce a tick */
				timerObj->alive_timeout -= PCD_TIMER_TICK;
			}
    	}
    }
